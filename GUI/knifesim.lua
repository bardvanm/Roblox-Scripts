    ...
    if AimbotToggle == false then
        ...
        local function findNearestPlayer()
            ...
            for _, player in pairs(playersService:GetPlayers()) do
                if player ~= localPlayer then
                    ...
                    if character then
                        ...
                        if primaryPart then
                            local distance = (mouse.Hit.Position - primaryPart.Position).Magnitude
            ...
            if nearestPlayer then
                return nearestPlayer.Character.PrimaryPart.CFrame.Position
            else
                return nil
            end
        end
...
OpenAimbot.MouseButton1Click:Connect(function()
    ...
    if AimbotToggle then
        ...
        local function findNearestPlayer()
            local nearestPlayer, shortestDistance = nil, 5000
...
CloseESP.MouseButton1Click:connect(function()
    ...
    local function highlightModel(objObject)
        for _, child in pairs(objObject:GetChildren()) do
            if child:IsA("BasePart") and child.Name ~= "HumanoidRootPart" then
                local bHA = Instance.new("BoxHandleAdornment", child)
                bHA.Adornee = child
                bHA.Size = (child.Name == "Head") and Vector3.new(1.25, 1.25, 1.25) or child.Size
                bHA.Color3 = (child.Name == "Head" and Color3.new(1, 0, 0)) or (child.Name == "Torso" and Color3.new(0, 1, 0)) or Color3.new(0, 0, 1)
                bHA.Transparency = 0
                bHA.ZIndex = 1
...
local function teleportToPlayer(player)
    ...
    until player.Character.Humanoid.Health == 0 or not tpKillToggle
end

local function findNearestPlayer()
    local players = game:GetService("Players")
    local mouse = players.LocalPlayer:GetMouse()
    local nearestPlayer, nearestDistance = nil, 5000
    
    for _, player in pairs(players:GetPlayers()) do
        local pos = player.Character and player.Character:GetPrimaryPartCFrame().p
        if pos then
            local len = (mouse.Hit.p - pos).Magnitude
            if len <= nearestDistance then
                nearestPlayer = player
                nearestDistance = len
            end
        end
    ...
    return nearestPlayer and nearestPlayer.Character:GetPrimaryPartCFrame().p
end
...
local function activateTeleportTool(mouse)
